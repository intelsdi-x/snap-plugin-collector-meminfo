...
__deployment_name="meminfo-deployment"
...
kubectl exec $(kubectl get po --no-headers | grep $__deployment_name | grep Running | awk '{print $1}') -c meminfo-large-test -i  -- mkdir /src
tar c  . | kubectl exec $(kubectl get po --no-headers | grep $__deployment_name | grep Running | awk '{print $1}') -c meminfo-large-test -i  --  tar -x -C /src

set +e
_debug "running tests through the runner"
kubectl exec $(kubectl get po --no-headers | grep $__deployment_name | grep Running | awk '{print $1}') -c meminfo-large-test -i -- /src/scripts/large_test.sh
test_res=$?
set -e
